...
func addUsedChannel(c *gin.Context, channelId string) {
	useChannel := c.GetStringSlice("use_channel")
	useChannel = append(useChannel, fmt.Sprintf("%s", channelId))
...
func shouldRetryTaskRelay(c *gin.Context, channelId string, taskErr *dto.TaskError, retryTimes int) bool {
	if taskErr == nil {
		return false
	}
	if retryTimes <= 0 {
