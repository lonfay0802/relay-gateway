...
func UpdateVideoTaskAll(ctx context.Context, platform constant.TaskPlatform, taskChannelM map[string][]string, taskM map[string]*model.Task) error {
...
func updateVideoTaskAll(ctx context.Context, platform constant.TaskPlatform, channelId string, taskIds []string, taskM map[string]*model.Task) error {
				...
				if modelName, ok := taskData["model"].(string); ok && modelName != "" {
					...
					if hasRatioSetting && modelRatio > 0 {
						...
						if group != "" {
							...
							groupRatio, ok := model.GetDefaultGroupRatioByModelName(modelName)
							if !ok {
								groupRatio = 1.0
							}
							...
							if quotaDelta > 0 {
								...
								if err := model.DecreaseUserQuota(task.UserId, quotaDelta); err != nil {
									...
									// 传递正数 quotaDelta 表示扣费
									model.RecordLog(task.UserId, model.LogTypeSystem, logContent, quotaDelta)
								}
