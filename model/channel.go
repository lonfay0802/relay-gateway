...
type Channel struct {
	...
	CreatedTime        *time.Time `json:"created_time" gorm:"type:timestamptz(6);default:now()"`
	TestTime           *time.Time `json:"test_time" gorm:"type:timestamptz(6);default:now()"`
	ResponseTime       int        `json:"response_time"` // in milliseconds
	BaseURL            *string    `json:"base_url" gorm:"column:base_url;default:''"`
	Other              string     `json:"other"`
	Balance            float64    `json:"balance"` // in USD
	BalanceUpdatedTime *time.Time `json:"balance_updated_time" gorm:"type:timestamptz(6);default:now()"`
...
func (channel *Channel) UpdateResponseTime(responseTime int64) {
	err := DB.Model(channel).Select("response_time", "test_time").Updates(Channel{
		TestTime:     common.GetTimestampTz(),
...
func (channel *Channel) UpdateBalance(balance float64) {
	err := DB.Model(channel).Select("balance_updated_time", "balance").Updates(Channel{
		BalanceUpdatedTime: common.GetTimestampTz(),
		Balance:            balance,
	}).Error
	if err != nil {
...
func GetChannelPollingLock(channelId string) *sync.Mutex {
...
func UpdateChannelStatus(channelId string, usingKey string, status int, reason string) bool {
	if common.MemoryCacheEnabled {
...
func UpdateChannelUsedQuota(id string, quota int) {
