...
import (
	...
	"relay-gateway/dto"
	commonRelay "relay-gateway/relay/common"
)
...
type Task struct {
	...
	SubmitTime int64                 `json:"submit_time" gorm:"type:bigint"`
	StartTime  int64                 `json:"start_time" gorm:"type:bigint"`
	FinishTime int64                 `json:"finish_time" gorm:"type:bigint"`
...
func TaskGetAllUserTask(userId int, startIdx int, num int, queryParams SyncTaskQueryParams) []*Task {
	...
	// 初始化查询构建器
	query := DB.Table("t_tasks").Where("user_id = ?", userId)
...
func GetAllUnFinishSyncTasks(limit int) []*Task {
	...
	err = DB.Table("t_tasks").Where("progress != ?", "100%").Where("status != ?", TaskStatusFailure).Where("status != ?", TaskStatusSuccess).Limit(limit).Order("id").Find(&tasks).Error
	if err != nil {
		return nil
	}
	return tasks
...
func GetByOnlyTaskId(taskId string) (*Task, bool, error) {
	...
	var task *Task
	var err error
	err = DB.Table("t_tasks").Where("task_id = ?", taskId).First(&task).Error
...
func GetByTaskId(userId string, taskId string) (*Task, bool, error) {
	if taskId == "" {
		return nil, false, nil
	}
	var task *Task
	var err error
	err = DB.Table("t_tasks").Where("user_id = ? and task_id = ?", userId, taskId).
		First(&task).Error
	exist, err := RecordExist(err)
	if err != nil {
		return nil, false, err
...
func TaskBulkUpdateByID(ids []string, params map[string]any) error {
	if len(ids) == 0 {
		return nil
	}
	return DB.Model(&Task{}).
