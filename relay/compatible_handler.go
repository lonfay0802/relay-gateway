...
import (
	...
	"relay-gateway/relay/helper"
	"relay-gateway/service"
	"relay-gateway/setting/model_setting"
...
func postConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.Usage, extraContent string) {
	...
	textCacheTokens := usage.PromptTokensDetails.TextTokens
	audioCacheTokens := usage.PromptTokensDetails.AudioTokens
	modelName := relayInfo.OriginModelName

	dTextCacheTokens := decimal.NewFromInt(int64(textCacheTokens))
	dAudioCacheTokens := decimal.NewFromInt(int64(audioCacheTokens))
	...
	dAudioCacheRatio := decimal.NewFromFloat(audioCacheRatio)
	...
	var textCachedTokenWithRatio decimal.Decimal
	var audioCachedTokenWithRatio decimal.Decimal
	if !relayInfo.PriceData.UsePrice {
		baseTokens := dPromptTokens
		// 减去 cached tokens
		var cachedTokensWithRatio decimal.Decimal
		if !dCacheTokens.IsZero() {
			if dTextCacheTokens.IsZero() && dAudioCacheTokens.IsZero() {
				baseTokens = baseTokens.Sub(dCacheTokens)
				cachedTokensWithRatio = dCacheTokens.Mul(dCacheRatio)
			} else {
				if !dTextCacheTokens.IsZero() {
					baseTokens = baseTokens.Sub(dTextCacheTokens)
					textCachedTokenWithRatio = dTextCacheTokens.Mul(dCacheRatio)
				}
				if !dAudioCacheTokens.IsZero() {
					baseTokens = baseTokens.Sub(dAudioCacheTokens)
					audioCachedTokenWithRatio = dAudioCacheTokens.Mul(dAudioCacheRatio)
				}
			}
		}
		...
		promptQuota := baseTokens.Add(cachedTokensWithRatio).
			Add(textCachedTokenWithRatio).
			Add(audioCachedTokenWithRatio).
			Add(imageTokensWithRatio).
			Add(dCachedCreationTokensWithRatio)

		completionQuota := dCompletionTokens.Mul(dCompletionRatio).Mul(dGroupRatio)

		// modelRatio 只应用于输入部分，输出部分已经通过 completionRatio 处理
		quotaCalculateDecimal = promptQuota.Mul(ratio).Add(completionQuota)
	...
	logParams := model.RecordConsumeLogParams{
		...
		Other:            other,
		// 新增字段用于新表记录
		ModelID:      logModel, // 使用logModel作为model_id（如果需要从数据库查找，可以后续优化）
