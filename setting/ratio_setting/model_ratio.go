...
import (
	...
	"relay-gateway/setting/operation_setting"
)
...
var (
	inputTokenPriceMap      map[string]float64 = nil
	inputTokenPriceMapMutex                    = sync.RWMutex{}
)

var (
	outputTokenPriceMap      map[string]float64 = nil
	outputTokenPriceMapMutex                    = sync.RWMutex{}
)

var (
	modelBillingModeMap      map[string]string = nil
	modelBillingModeMapMutex                   = sync.RWMutex{}
)
...
// It first initializes with default values, then loads from database if available
...
// GetBillingMode 获取计费类型
func GetBillingMode(name string) string {
	modelBillingModeMapMutex.RLock()
	defer modelBillingModeMapMutex.RUnlock()
	name = FormatMatchingModelName(name)
	if ratio, ok := modelBillingModeMap[name]; ok {
...
// GetInputTokenPrice 获取输入token价格
func GetInputTokenPrice(name string) float64 {
	inputTokenPriceMapMutex.RLock()
	defer inputTokenPriceMapMutex.RUnlock()
	name = FormatMatchingModelName(name)
	if price, ok := inputTokenPriceMap[name]; ok {
		return price
	}
	return 0.0
}

// GetOutputTokenPrice 获取输出token价格
func GetOutputTokenPrice(name string) float64 {
	outputTokenPriceMapMutex.RLock()
	defer outputTokenPriceMapMutex.RUnlock()
	name = FormatMatchingModelName(name)
	if price, ok := outputTokenPriceMap[name]; ok {
		return price
	}
	return 0.0
}
...
func GetAudioCacheRatio(name string) float64 {
	audioCacheRatioMapMutex.RLock()
	defer audioCacheRatioMapMutex.RUnlock()
	name = FormatMatchingModelName(name)
	if ratio, ok := audioCacheRatioMap[name]; ok {
		return ratio
	}
	return 1
}
...
var (
	audioCacheRatioMap      map[string]float64 = nil
	audioCacheRatioMapMutex                    = sync.RWMutex{}
)
...
// UpdateModelRatioFromMap 从 map 更新模型倍率（用于从数据库加载）
func UpdateModelRatioFromMap(ratioMap map[string]float64) error {
	modelRatioMapMutex.Lock()
	defer modelRatioMapMutex.Unlock()

	// 如果 modelRatioMap 还未初始化，先初始化为默认值
...
// UpdateCompletionRatioFromMap 从 map 更新完成倍率（用于从数据库加载）
func UpdateCompletionRatioFromMap(ratioMap map[string]float64) error {
	CompletionRatioMutex.Lock()
	defer CompletionRatioMutex.Unlock()

	// 如果 CompletionRatio 还未初始化，先初始化为默认值
...
// UpdateModelPriceFromMap 从 map 更新模型价格（用于从数据库加载）
func UpdateModelPriceFromMap(priceMap map[string]float64) error {
	modelPriceMapMutex.Lock()
	defer modelPriceMapMutex.Unlock()

	// 如果 modelPriceMap 还未初始化，先初始化为默认值
...
// UpdateImageRatioFromMap 从 map 更新图片倍率（用于从数据库加载）
func UpdateImageRatioFromMap(ratioMap map[string]float64) error {
	imageRatioMapMutex.Lock()
	defer imageRatioMapMutex.Unlock()

	// 如果 imageRatioMap 还未初始化，先初始化为默认值
	if imageRatioMap == nil {
		imageRatioMap = make(map[string]float64)
		// 复制默认值
		for k, v := range defaultImageRatio {
			imageRatioMap[k] = v
		}
	}
...
// UpdateAudioRatioFromMap 从 map 更新音频倍率（用于从数据库加载）
func UpdateAudioRatioFromMap(ratioMap map[string]float64) error {
	audioRatioMapMutex.Lock()
	defer audioRatioMapMutex.Unlock()

	// 如果 audioRatioMap 还未初始化，先初始化为默认值
	if audioRatioMap == nil {
		audioRatioMap = make(map[string]float64)
		// 复制默认值
		for k, v := range defaultAudioRatio {
			audioRatioMap[k] = v
		}
	}
...
// UpdateAudioCompletionRatioFromMap 从 map 更新音频完成倍率（用于从数据库加载）
func UpdateAudioCompletionRatioFromMap(ratioMap map[string]float64) error {
	audioCompletionRatioMapMutex.Lock()
	defer audioCompletionRatioMapMutex.Unlock()

	// 如果 audioCompletionRatioMap 还未初始化，先初始化为默认值
	if audioCompletionRatioMap == nil {
		audioCompletionRatioMap = make(map[string]float64)
		// 复制默认值
		for k, v := range defaultAudioCompletionRatio {
			audioCompletionRatioMap[k] = v
		}
	}
...
// UpdateModelBillingModeFromMap 从 map 更新模型计费模式（用于从数据库加载）
func UpdateModelBillingModeFromMap(billingMap map[string]string) error {
	modelBillingModeMapMutex.Lock()
	defer modelBillingModeMapMutex.Unlock()

	// 如果 modelBillingModeMap 还未初始化，先初始化为空 map
	if modelBillingModeMap == nil {
		modelBillingModeMap = make(map[string]string)
	}

	// 更新或添加数据库中的值
	for k, v := range billingMap {
		modelBillingModeMap[k] = v
	}
...
// UpdateInputTokenPriceFromMap 从 map 更新输入 token 价格（用于从数据库加载）
func UpdateInputTokenPriceFromMap(priceMap map[string]float64) error {
	inputTokenPriceMapMutex.Lock()
	defer inputTokenPriceMapMutex.Unlock()

	// 如果 inputTokenPriceMap 还未初始化，先初始化为空 map
	if inputTokenPriceMap == nil {
		inputTokenPriceMap = make(map[string]float64)
	}

	// 更新或添加数据库中的值
	for k, v := range priceMap {
		inputTokenPriceMap[k] = v
	}
...
// UpdateOutputTokenPriceFromMap 从 map 更新输出 token 价格（用于从数据库加载）
func UpdateOutputTokenPriceFromMap(priceMap map[string]float64) error {
	outputTokenPriceMapMutex.Lock()
	defer outputTokenPriceMapMutex.Unlock()

	// 如果 outputTokenPriceMap 还未初始化，先初始化为空 map
	if outputTokenPriceMap == nil {
		outputTokenPriceMap = make(map[string]float64)
	}

	// 更新或添加数据库中的值
	for k, v := range priceMap {
		outputTokenPriceMap[k] = v
	}
